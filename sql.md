...
#for exportlist in  `cat xxx_list` 

for exportlist in `db2 -x -o "select trim(tabname) from syscat.tables where trim(tabschema) = 'schema' order by tabname "`

do

​       schema_table=`echo  "${exportlist}" |awk -F"." '{print $1}'"`

​     echo "================${schema_table}====================="

​     echo " db2 export to ${schema_table}.ixf of ixf select * from schema.${schema_table}"

​     db2 “export to ${schema_table}.ixf of ixf select * from schema.${schema_table}”
...
​     echo " db2 load from ${schema_table}.ixf of ixf replace into schema.${schema_table}"

​     db2 “load from ${schema_table}.ixf of ixf replace into schema.${schema_table}”
